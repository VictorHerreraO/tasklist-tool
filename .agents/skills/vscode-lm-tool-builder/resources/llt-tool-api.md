...
Language model tools enable you to extend the functionality of a large language model (LLM) in chat with domain-specific capabilities. To process a user's chat prompt, agents in VS Code can automatically invoke these tools to perform specialized tasks as part of the conversation.

By contributing a language model tool in your VS Code extension, you can extend the agentic coding workflow while also providing deep integration with the editor. Extension tools are one of three types of tools available in VS Code, alongside built-in tools and MCP tools.
...
You can also extend the chat experience with specialized tools by contributing an MCP server. See the [AI Extensibility Overview](./ai-extensibility.md) for details on the different options and how to decide which approach to use.

> [!Tip]
> For information about using tools as an end user, see Use tools in chat.
...
flowchart TD
    ...
    MCP_Perp("MCP Server\n(e.g. Perplexity)")
    MCP_Git("MCP Server\n(e.g. GitHub)")
    VSC_Mermaid("VS Code Extension\n(e.g. Mermaid Chart)")

    %% Loop Subgraph (Orange)
    subgraph Loop ["Loop"]
        LLM("LLM")
    end

    %% Tools Subgraph (Blue & Green)
    subgraph Tools ["Tools"]
        read_file(["read_file"])
        edit_file(["edit_file"])
        run_in_terminal(["run_in_terminal"])

        subgraph Extensible ["Extensible: contributed tools"]
    ...
    %% Interactions (Black lines)
    User -- "Prompt" --> Copilot
    Copilot -- "Streamed result" --> User
    Machine -- "Summarized workspace\nand OS context" --> Copilot

    %% Interactions (Orange lines)
    Copilot -- "Prompt+ context + tool description +\n(tool call result)" --> LLM
    LLM -- "Result or decision to call tool with args" --> Copilot

    %% Interactions (Blue lines)
    Copilot -- "Tool call" --> Tools
    Tools --> Machine

    %% Interactions (Green lines)
    MCP_Perp -- "Contributes tools" --> perplexity_search
    MCP_Git -- "Contributes tools" --> get_issues
    VSC_Mermaid -- "Contributes tools" --> open_diagram


    %% Styling to match diagram colors
    style Loop stroke:#F59E0B,stroke-width:2px,fill:none,color:#F59E0B
    style LLM stroke:#F59E0B,stroke-width:2px,fill:none,color:#F59E0B
    
    style Tools stroke:#3B82F6,stroke-width:2px,fill:none,color:#3B82F6
    style read_file stroke:#3B82F6,stroke-width:2px,fill:none,color:#3B82F6
    style edit_file stroke:#3B82F6,stroke-width:2px,fill:none,color:#3B82F6
    style run_in_terminal stroke:#3B82F6,stroke-width:2px,fill:none,color:#3B82F6

    style Extensible stroke:#22C55E,stroke-width:2px,fill:none,color:#22C55E
    style MCP_Perp stroke:#22C55E,stroke-width:2px,fill:none,color:#22C55E
    style MCP_Git stroke:#22C55E,stroke-width:2px,fill:none,color:#22C55E
    style VSC_Mermaid stroke:#22C55E,stroke-width:2px,fill:none,color:#22C55E
    style get_issues stroke:#22C55E,stroke-width:2px,fill:none,color:#22C55E
    style perplexity_search stroke:#22C55E,stroke-width:2px,fill:none,color:#22C55E
    style open_diagram stroke:#22C55E,stroke-width:2px,fill:none,color:#22C55E
```
...
3. If the tool can be used with agents or referenced in a chat prompt with #, add the following properties:

Users can enable or disable the tool in the Chat view, similar to how this is done for Model Context Protocol (MCP) tools.

| Property                | Description                                                            |
| ----------------------- | ---------------------------------------------------------------------- |
| canBeReferencedInPrompt | Set to true if the tool can be used with agents or referenced in chat. |
